...
//    3.传递一个实现了此接口的类的对象（至备份短信的工具类中），接口的实现类，一定实现了上诉两个未实现方法（就决定了使用对话框还是进度条
//    4.获取传递进来的对象，在合适的地方（设置总数，设置百分比的地方）做方法的调用
    public interface CallBack {
        // 短信总数设置为实现方法（由自己决定是用对话框.setMax(max)还是用进度条.setMax(max)）
        void setMax(int max);
        // 备份过程中短信百分比更新（由自己决定使用 对话框.setProgress(max)还是用进度条.setProgress(max)）
